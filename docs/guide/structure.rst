...
``[[./topics/example/basics]]`` — is an issue too, naming the prefix-free
``[[topics/example/basics]]`` form: every target is read from the wiki root,
and a prefixed target must leave the wiki (see :doc:`/configuration`).
