...
__all__ = [
    ...
    'test_write_atomic_unencodable_discards_temp',
]
...
def test_write_atomic_failure_discards_temp(
    ...
    def refuse(src: pathlib.Path, dst: pathlib.Path) -> None:
...
def test_write_atomic_unencodable_discards_temp(tmp_path: pathlib.Path) -> None:
    """A failure while writing the temp file leaves nothing behind.

    Unencodable text raises before the swap, so the temp file is
    discarded and no target is created.
    """
    target = tmp_path / 'page.md'
    with pytest.raises(UnicodeEncodeError):
        write_atomic(target, 'lone surrogate \ud800\n')
