...
__all__ = [
    ...
    'test_markerless_child_index_warns_when_mapping_from_root',
...
def test_map_survives_readonly_cache(tmp_path: pathlib.Path) -> None:
    """An unwritable cache directory degrades to a recompute, never an error.

    The cache is a pure optimization: when the flush fails (a read-only
    checkout, a cache directory owned by another user), the command still
    renders real counts and simply recomputes on the next run.
    """
    wiki = _make_wiki(tmp_path, folders={'core': ['design']})
    # stale the cache, then take the flush away
...
def test_map_recounts_same_mtime_rewrite(tmp_path: pathlib.Path) -> None:
    """A rewrite landing within the mtime granularity is still recounted.

    Freshness keys on size as well as mtime, so an edit that leaves the
    timestamp unchanged (a same-tick autosave, a coarse-granularity
    filesystem) cannot pin a stale count forever.
    """
    wiki = _make_wiki(tmp_path, folders={'core': ['design']})
    page = tmp_path / 'core' / 'design.md'
    before = page.stat()
    # rewrite the body, then put the original timestamp back
    page.write_text(
        page.read_text(encoding='utf-8') + '\nMany more words for the count.\n',
        encoding='utf-8',
    )
    os.utime(page, ns=(before.st_atime_ns, before.st_mtime_ns))

    # the render recounts the page rather than serving the cached total
    cache = tmp_path / '.wiki' / 'cache' / 'word_counts.json'
    counts = json.loads(cache.read_text(encoding='utf-8'))
    stale_words = counts['core/design.md']['words']
    match = re.search(r'design \((\d+)\)', wiki.map())
    assert match
    assert int(match.group(1)) > stale_words
...
def test_markerless_child_index_warns_when_mapping_from_root(
    tmp_path: pathlib.Path,
) -> None:
    """A damaged child index is named by a root-scoped map, not only a scoped one.

    map warns about a markerless index so a damaged tree is not read silently
    from the reclaim; the warning must cover child indexes the recursion
    parses, not just the scope root the user happened to name.
    """
    wiki = _make_wiki(tmp_path, folders={'core': ['design']})
    # drop the CHILD index's *** delimiter (its pages remain on disk)
    child_index = tmp_path / 'core' / '_index.md'
    text = child_index.read_text(encoding='utf-8')
    text = '\n'.join(line for line in text.split('\n') if line.rstrip() != '***')
    child_index.write_text(text, encoding='utf-8')

    # a root-scoped map warns about the damaged child, naming it
    notices = _capture_notices(wiki)
    wiki.map()
    err = '\n'.join(event.description for event in notices)
    assert 'core/_index.md is missing its *** delimiter' in err
...
def test_map_marks_case_renamed_target_broken(tmp_path: pathlib.Path) -> None:
